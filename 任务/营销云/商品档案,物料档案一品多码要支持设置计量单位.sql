...
set item.cCaption='条码',item.cShowCaption='条码',cControlType='Input',cStyle='{"placeholder":"条码"}'
...
select item.cSubId,item.iBillId,'unitId','unitId','条码计量单位id','计量单位id',item.iBillEntityId,tpl.id,item.iTplId,item.iOrder+1,255,1,null,null,1,1,null,null,null,null,null,0,0,null,1,null,255,null,1,null,1,0,1,null,1,null,null,null,1,1,1,0,0,0,null,item.cDataSourceName,NULL,null,'Input',null,0,null,null,0,item.bVmExclude,item.tenant_id,item.multiple,item.depends, item.cDefineName, item.makeField, item.cSensFieldName
...
# ListRefer
insert into billitem_base(cSubId, iBillId, cFieldName, cName, cCaption, cShowCaption, iBillEntityId, iBillTplGroupId, iTplId, iOrder, iMaxLength, iFieldType, bEnum, cEnumString, bMustSelect, bHidden, cRefType, cRefId, cRefRetId, cDataRule, iFunctionType, bSplit, bExtend, iNumPoint, bCanModify, cSourceType, iMaxShowLen, cMemo, iColWidth, cSumType, iAlign, bNeedSum, bShowIt, bFixed, bFilter, cDefaultValue, cFormatData, cUserId, iTabIndex, bIsNull, bPrintCaption, bJointQuery, bPrintUpCase, bSelfDefine, cSelfDefineType, cDataSourceName, cOrder, bCheck, cControlType, refReturn, bShowInRowAuth, iRowAuthBillId, cStyle, bRowAuthControlled,bVmExclude,tenant_id, multiple, depends, cDefineName, makeField, cSensFieldName)
select item.cSubId,item.iBillId,'unitId.name','unitName','条码计量单位名称','',item.iBillEntityId,tpl.id,item.iTplId,item.iOrder+2,255,1,null,null,1,0,'pc_unitref',null,'{"unitId":"id","unitName":"name"}',null,null,0,0,null,1,null,255,null,1,null,1,0,1,null,1,null,null,null,1,1,1,0,0,0,null,item.cDataSourceName,NULL,null,'Refer','name',0,null,'{"bottom":"请先选择下面的计量单位！","tips":"请先选择下面的计量单位！"}',0,item.bVmExclude,item.tenant_id,0,item.depends, item.cDefineName, item.makeField, item.cSensFieldName
from billitem_base item inner join bill_base bill on item.iBillId=bill.id and item.tenant_id=bill.tenant_id inner join billtplgroup_base tpl on tpl.iBillId=bill.id
...
select 'pc_product','barcodeUnit',1,'productBarCodeDiv','right','PC','productBarCodes',null,bill_base.tenant_id
...
select 'pc_product','barcodeUnit','btnAddRow_barcodeUnit','cmdAddRow_barcodeUnit',null,'button',0,null,null,null,null,1,'PC',1,1,'tianjiashuliang',bill_base.tenant_id,5
from bill_base where cBillNo='pc_product' and not exists(select 1 from bill_toolbaritem where name='btnAddRow_barcodeUnit');

insert into bill_toolbaritem(billnumber, toolbar, name, command, authid, type, style, text, imgsrc, parent, parameter, `order`, subid, `system`, authcontrol, icon, tenant_id, auth_level)
select 'pc_product','barcodeUnit','btnDeleteRow_barcodeUnit','cmdDeleteRow_barcodeUnit',null,'button',0,null,null,null,null,2,'PC',1,1,'jianshaoshuliang',bill_base.tenant_id,5
...
'cmdAddRow_barcodeUnit','insertRow','pc_product','barcodeUnit',null,'{"direction":"down"}','/bill/insertRow','GET','AA',null,bill_base.tenant_id
...
select *
from billtplgroup_base where cType='PromotionGroup' and tenant_id=0;